...
class RedBlackTree:
    ...
    def fix_violations(self, violating_node):
        ...
        if uncle is None or uncle.color == RBNode.BLACK:
            if parent.is_right_node() and violating_node.is_right_node():
                self.left_rotate(grandparent)
                grandparent.toggle_color()
                parent.toggle_color()
            elif parent.is_right_node() and violating_node.is_left_node():
                self.right_rotate(parent)
                if parent.color == RBNode.RED:
                    fix_violations(parent)

            elif parent.is_left_node() and violating_node.is_left_node():
                self.right_rotate(grandparent)
                grandparent.toggle_color()
                parent.toggle_color()
            elif parent.is_left_node() and violating_node.is_right_node():
